...
class Configurations(object):
    ...
    def check_compatability(self):
        ...
        if len(self.RUN.eval_metrics):
            ...
            if any(item in ["is", "fid"] for item in self.RUN.eval_metrics)\
                and self.DATA.name == "MNIST"\
                and self.RUN.eval_backbone != "MNIST_ResNet18_torch":
                    print("Backbones other than MNIST_ResNet18_torch don't give meaningful FID and IS scores."\
                        "Forcing eval_backbone = MNIST_ResNet18_torch ...")
                    self.RUN.eval_backbone = "MNIST_ResNet18_torch"
        ...
        if self.DATA.name in ["CIFAR10", "CIFAR100", "MNIST"]:
        ...
        assert self.RUN.eval_backbone in ["InceptionV3_tf", "InceptionV3_torch", "ResNet50_torch", "SwAV_torch", "DINO_torch", "Swin-T_torch", "MNIST_ResNet18_torch"], \
            "eval_backbone should be in [InceptionV3_tf, InceptionV3_torch, ResNet50_torch, SwAV_torch, DINO_torch, Swin-T_torch, MNIST_ResNet18_torch]"
        ...
        assert self.RUN.data_dir is not None or self.RUN.save_fake_images, "Please specify data_dir if dataset is prepared. \
            \nIn the case of CIFAR10, CIFAR100 or MNIST, just specify the directory where you want \
