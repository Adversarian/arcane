...
def build_G_D(cfg, device="cuda"):  # TODO: if results are bad look at mixed precision
    module = __import__(
        "models.{backbone}".format(backbone=cfg.MODEL.backbone), fromlist=["something"]
    )
...
def load_generator_discriminator(
    DATA, OPTIMIZATION, MODEL, STYLEGAN, MODULES, RUN, device, logger
):
    if device == 0:
        logger.info("Build a Generative Adversarial Network.")
    module = __import__(
        "models.{backbone}".format(backbone=MODEL.backbone), fromlist=["something"]
    )
    if device == 0:
        logger.info(
            "Modules are located on './src/models.{backbone}'.".format(
    ...
    if MODEL.backbone in ["stylegan2", "stylegan3"]:
        channel_base, channel_max = (
            32768
            if MODEL.backbone == "stylegan3"
            or DATA.img_size >= 512
            or DATA.name in ["CIFAR10", "CIFAR100"]
            else 16384,
            512,
        )
        ...
        if MODEL.backbone == "stylegan2":
            ...
            magnitude_ema_beta = 0.5 ** (
                OPTIMIZATION.batch_size * OPTIMIZATION.acml_steps / (20 * 1e3)
            )
            g_channel_base, g_channel_max, conv_kernel, use_radial_filters = (
                channel_base,
                channel_max,
                3,
                False,
        ...
        module = __import__(
            "models.stylegan2", fromlist=["something"]
        )  # always use StyleGAN2 discriminator
        Dis = module.Discriminator(
            c_dim=dis_c_dim,
            img_resolution=DATA.img_size,
            img_channels=DATA.img_channels,
            ...
            block_kwargs={},
            mapping_kwargs={},
            epilogue_kwargs={"mbstd_group_size": STYLEGAN.d_epilogue_mbstd_group_size},
            MODEL=MODEL,
        ).to(device)
        ...
        if MODEL.apply_g_ema:
            if device == 0:
                logger.info(
                    "Prepare exponential moving average generator with decay rate of {decay}.".format(
...
def prepare_parallel_training(
    ...
    if distributed_data_parallel:
        if synchronized_bn:
            ...
            if apply_g_ema:
                Gen_ema = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
                    Gen_ema, process_group
                )

        if MODEL.backbone in ["stylegan2", "stylegan3"]:
            Gen_mapping = DDP(Gen.mapping, device_ids=[device], broadcast_buffers=False)
            Gen_synthesis = DDP(
                Gen.synthesis, device_ids=[device], broadcast_buffers=False
            )
        else:
        ...
        Dis = DDP(
            ...
            if MODEL.backbone in ["stylegan2", "stylegan3"]
            else synchronized_bn,
            find_unused_parameters=True
            if MODEL.info_type in ["discrete", "continuous", "both"]
            else False,
        )
        if apply_g_ema:
            if MODEL.backbone in ["stylegan2", "stylegan3"]:
                Gen_ema_mapping = DDP(
                    Gen_ema.mapping, device_ids=[device], broadcast_buffers=False
                )
                Gen_ema_synthesis = DDP(
                    Gen_ema.synthesis, device_ids=[device], broadcast_buffers=False
                )
            else:
                Gen_ema = DDP(
                    Gen_ema, device_ids=[device], broadcast_buffers=synchronized_bn
                )
    else:
        if MODEL.backbone in ["stylegan2", "stylegan3"]:
        ...
        if apply_g_ema:
            if MODEL.backbone in ["stylegan2", "stylegan3"]:
                ...
                Gen_ema_synthesis = DataParallel(
                    Gen_ema.synthesis, output_device=device
                )
            else:
