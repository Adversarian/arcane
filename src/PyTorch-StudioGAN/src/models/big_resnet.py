...
class GenBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, g_cond_mtd, affine_input_dim, MODULES
    ):
...
class Generator(nn.Module):
    def __init__(
        ...
        assert (
            self.z_dim % (self.num_blocks + 1) == 0
        ), "z_dim should be divided by the number of blocks"
        ...
        if self.MODEL.info_type != "N/A":
            if self.MODEL.g_info_injection == "concat":
                ...
                self.info_proj_linear = MODULES.g_linear(
                    in_features=info_dim, out_features=self.g_shared_dim, bias=True
                )

        self.linear0 = MODULES.g_linear(
            in_features=self.chunk_size,
        ...
        if self.g_cond_mtd != "W/O":
            self.affine_input_dim += self.g_shared_dim
            self.shared = ops.embedding(
                num_embeddings=self.num_classes, embedding_dim=self.g_shared_dim
            )
        ...
        for index in range(self.num_blocks):
            ...
            if index + 1 in attn_g_loc and apply_attn:
                self.blocks += [
                    [
                        ops.SelfAttention(
                            self.out_dims[index], is_generator=True, MODULES=MODULES
                        )
                    ]
                ]
        ...
        self.activation = MODULES.g_act_fn
        self.conv2d5 = MODULES.g_conv2d(
            in_channels=self.out_dims[-1],
            out_channels=DATA.img_channels,
            kernel_size=3,
            stride=1,
            padding=1,
        )
...
class Discriminator(nn.Module):
    def __init__(
        ...
        d_in_dims_collection = {
            "32": [DATA.img_channels]
            + [d_conv_dim * 2, d_conv_dim * 2, d_conv_dim * 2],
            "64": [DATA.img_channels]
            + [d_conv_dim, d_conv_dim * 2, d_conv_dim * 4, d_conv_dim * 8],
            "128": [DATA.img_channels]
            + [
                d_conv_dim,
                d_conv_dim * 2,
                d_conv_dim * 4,
        ...
        for index in range(len(self.in_dims)):
            ...
            if index + 1 in attn_d_loc and apply_attn:
                self.blocks += [
                    [
                        ops.SelfAttention(
                            self.out_dims[index], is_generator=False, MODULES=MODULES
                        )
                    ]
                ]
        ...
        if self.d_cond_mtd == "MH":
            self.linear1 = MODULES.d_linear(
                in_features=self.out_dims[-1], out_features=1 + num_classes, bias=True
            )
        elif self.d_cond_mtd == "MD":
            self.linear1 = MODULES.d_linear(
                in_features=self.out_dims[-1], out_features=num_classes, bias=True
            )
        else:
            self.linear1 = MODULES.d_linear(
                in_features=self.out_dims[-1], out_features=1, bias=True
            )
        ...
        if self.d_cond_mtd == "AC":
            self.linear2 = MODULES.d_linear(
                in_features=self.out_dims[-1], out_features=num_classes, bias=False
            )
        elif self.d_cond_mtd == "PD":
            self.embedding = MODULES.d_embedding(num_classes, self.out_dims[-1])
        elif self.d_cond_mtd in ["2C", "D2DCE"]:
            self.linear2 = MODULES.d_linear(
                in_features=self.out_dims[-1], out_features=d_embed_dim, bias=True
            )
        ...
        if self.MODEL.info_type in ["discrete", "both"]:
            ...
            self.info_discrete_linear = MODULES.d_linear(
                in_features=self.out_dims[-1], out_features=out_features, bias=False
            )
        if self.MODEL.info_type in ["continuous", "both"]:
    ...
    def forward_emb(self, x):
        ...
        if not hasattr(self, 'all_proxies'):
            self.all_proxies = self.embedding(torch.arange(0, num_classes, device="cuda"))
            if self.normalize_d_embed:
                self.all_proxies = F.normalize(self.all_proxies, dim=1)
        if self.normalize_d_embed:
            embed = F.normalize(embed, dim=1)
        cls_output = cosine_sim(embed[:,None,:], self.all_proxies)
