...
from metrics.resnet import ResNet
...
MNIST_RESNET18_URL = "https://raw.githubusercontent.com/sundyCoder/IS_MS_SS/master/ms/checkpoints/mnist_model_10.ckpt"
...
class LoadEvalModel(object):
    def __init__(self, eval_backbone, post_resizer, world_size, distributed_data_parallel, device):
        ...
        if self.eval_backbone == "InceptionV3_tf":
            ...
            self.model = self.model.to(self.device)
        elif self.eval_backbone == "MNIST_ResNet18_torch":
            self.res, mean, std = 28, [0.1307], [0.3081]
            self.model = ResNet("MNIST", 18, 10)
            model_state_dict = load_state_dict_from_url(MNIST_RESNET18_URL, progress=True)
            # layer names from the downloaded state_dict differ from StudioGAN's implementation
            # we have to forcefully match the layer names here to be able to load the state_dict
            # taken from: https://discuss.pytorch.org/t/loading-weights-from-pretrained-model-with-different-module-names/11841
            new_weights = list(model_state_dict.items())
            constructed_state_dict = self.model.state_dict()
            count = 0
            for key,_ in constructed_state_dict.items():
                _, weights = new_weights[count]
                constructed_state_dict[key] = weights.detach().clone()
                count += 1
            self.model.load_state_dict(constructed_state_dict, strict=True)
            self.model = self.model.to(self.device)
            hook_handles = []
            for name, layer in self.model.named_children():
                if name == "fc":
        ...
        shape = (1, 3, 1, 1)
        if self.eval_backbone == "MNIST_ResNet18_torch":
            shape = (1, 1, 1, 1)
        self.mean = torch.Tensor(mean).view(*shape).to(self.device)
        self.std = torch.Tensor(std).view(*shape).to(self.device)
    ...
    def get_outputs(self, x, quantize=False):
        ...
        elif self.eval_backbone in ["InceptionV3_torch", "ResNet50_torch", "SwAV_torch", "MNIST_ResNet18_torch"]:
            logits = self.model(x)
